//Usar objeto no lugar de if e else pra verificar mensagem de erro é mais eficiente
...
const APIKey = 'bcc2a68f24c8e8d9ec14e172'
...
const getErrormessage = errorType => ({
    ...
    'malformed-request': 'O endpoint do seu request precisa seguir a estrutura à seguir: https://v6.exchangerate-api.com/v6/bcc2a68f24c8e8d9ec14e172/latest/',
    'invalid-key': 'A chave da API não é válida.',
    'quota-reached': 'Sua cota alcançou o limite de requests permitido em seu plano atual.',
    'inactive-account': 'E-mail da API não confirmado.'
...
const getOptions = (selectedCurrency, conversion_rates) => {
    const setSelectedAttibute = currency =>
        currency === selectedCurrency ? 'selected' : ''
    const getOptionsAsArray = currency =>
        `<option ${setSelectedAttibute(currency)}>${currency}</option>`
